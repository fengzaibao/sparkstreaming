...
object SegmentService extends Serializable {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //单次请求测试
    //    val segments=SegmentService.segment("http://localhost:8283/token/","今天是星期天,今天天气是不错")
    //    print(  segments )
    //如有网络抖动的情况下，重新联接三次.
    //    val segments = SegmentService.retry(3)(SegmentService.segment("http://localhost:8283/token/", "今天是星期天,今天天气是不错"))
    //    print(segments)
    //3. 对拆分出来的词在用户词典表中 查找，并计数
    val record="今天是星期天,今天天气是不错,是的,你觉得呢,是"
    val wordDic=new HashSet[String]()
    wordDic.add("是")
    wordDic.add( "今天")
    wordDic.add("星期天")
    val map=mapSegment(   record, wordDic)
    val a=segment("http://localhost:8282/token/","今天是星期天,今天天气是不错")
    print( map )
  }
  ...
  def mapSegment(record: String, wordDic: HashSet[String]): Map[String, Int] = {
    ...
    if (record == "" || record.isEmpty()) {
      ...
      try {
        ...
        //val keyCount = Map[String, Int]()
        for (word <- wordDic) {
          if (wordsSet.contains(word)) //这里是判断用户字典表里的词有没有和切分的次一样
            keyCount += word -> 1
        }
